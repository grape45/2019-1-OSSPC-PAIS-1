		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				case KeyEvent.VK_S:
					...
					// Hold function
				case KeyEvent.VK_H:
					if (board.isValidAndEmpty(currentType, currentCol, currentRow + 1, currentRotation) && holdCount == 0)
						{
							if (holdType == null)
							{
					...
					/*
					 * Quick Drop - When pressed
					 * paused and that there is no drop cooldown, then set the
					 * logic timer to run at a speed of 25 cycles per second.
					 */
				case KeyEvent.VK_Z:
					if(!isPaused && dropCooldown == 0) {
						while (board.isValidAndEmpty(currentType, currentCol, currentRow + 1, currentRotation))
								currentRow++;
						logicTimer.setCyclesPerSecond(25.0f);
					}
					break;
